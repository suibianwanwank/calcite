...
public class JoinRexUtils {
  ...
  public static RexNode addJoinDerivePredicate(RexBuilder rexBuilder,
      JoinContext joinContext, @Nullable RexNode rexNode) {
    ...
    rexNode = rexNode == null ? derivedPredicateOpt.get()
        : rexBuilder.makeCall(SqlStdOperatorTable.AND, rexNode, derivedPredicateOpt.get());
    return compensateJoinNullTypeColumn(rexNode, rexBuilder,
        joinContext.getTargetDataTypeList(), joinContext.getNeedCompensateNullColumn());
  }
  public static boolean canEliminateNull() {
    return true;
  }
  private static Optional<RexNode> getJoinDerivedPredicate(RexBuilder rexBuilder,
      List<RexInputRef> candidateColumns, JoinContext joinContext, boolean isNeedCompensate) {
  ...
  public static boolean isJoinRewritable(JoinContext joinContext, int leftIdx) {
    ...
    case INNER_JOIN_TO_RIGHT_JOIN :
      return true;
    case LEFT_JOIN_TO_FULL_JOIN :
      return hasNotNullColumn(
          relDataTypeFieldList.subList(
              0, leftIdx), joinContext, 0);
    case RIGHT_JOIN_TO_FULL_JOIN :
      return hasNotNullColumn(
          relDataTypeFieldList.subList(
          leftIdx, relDataTypeFieldList.size()), joinContext, leftIdx);
    case INNER_JOIN_TO_FULL_JOIN :
      return hasNotNullColumn(
          relDataTypeFieldList.subList(
          leftIdx, relDataTypeFieldList.size()), joinContext, leftIdx)
          && hasNotNullColumn(relDataTypeFieldList.subList(0, leftIdx), joinContext, 0);
    default :
      return false;
    }
  }
  public static boolean hasNotNullColumn(
      List<RelDataTypeField> columnList, JoinContext joinContext, int offset) {
    for (RelDataTypeField column : columnList) {
      if (!column.getType().isNullable()) {
        joinContext.getNotNullColumns().add(
            new RexInputRef(column.getIndex() + offset, column.getType()));
        return true;
      }
    }
    return false;
  ...
  public static boolean isSupportedPredict(
      RexNode joinCondition, List<Pair<RexInputRef, RexInputRef>> joinColumnPairs,
  ...
  public static boolean isColumnEqualBinaryPredicate(RexNode predicate) {
    if (predicate instanceof RexCall) {
      ...
      return binaryPredicate.getOperator().equals(SqlStdOperatorTable.EQUALS)
          && binaryPredicate.getOperands()
          .stream()
          .allMatch(rexNode -> rexNode instanceof RexInputRef);
    }
    return false;
  }
  public static @Nullable JoinContext extractJoinContext(
      MutableJoin queryRel, MutableJoin targetRel,
      List<Pair<RexInputRef, RexInputRef>> joinColumnPairs,
      int joinDivision, RexBuilder rexBuilder) {
    requireNonNull(joinColumnPairs, "joinColumnPairs");
    ...
    Pair<List<RexInputRef>, List<RexInputRef>> joinColumnPair =
        Pair.of(joinColumnPairs.stream().map(Pair::getKey).collect(Collectors.toList()),
            joinColumnPairs.stream().map(Pair::getValue).collect(Collectors.toList()));
    List<Integer> needCompensateNullColumn =
        parseNeedCompensateNullColumn(joinRewriteType, targetRel, joinDivision, rexBuilder);
    List<RelDataType> targetDataTypeList =
        targetRel.rowType.getFieldList().stream()
            .map(Map.Entry::getValue)
            .collect(Collectors.toList());
    List<RelDataTypeField> inputDataTypeList =
        Stream.concat(targetRel.getLeft().rowType.getFieldList().stream(),
            targetRel.getRight().rowType.getFieldList().stream()).collect(Collectors.toList());
    return new JoinContext(joinRewriteType, needCompensateNullColumn,
        joinColumnPair, targetDataTypeList, inputDataTypeList);
  }
  public static List<Integer> parseNeedCompensateNullColumn(
      JoinRewriteType type, MutableJoin targetRel, int joinDivision, RexBuilder rexBuilder) {
    List<Integer> targetRelDataTypeIndexList = targetRel.rowType.getFieldList().stream()
        .map(RelDataTypeField::getIndex)
        .collect(Collectors.toList());
    switch (type) {
    case INNER_JOIN_TO_LEFT_JOIN :
    case RIGHT_JOIN_TO_FULL_JOIN :
      return getNeedCompensateNullColumn(
          targetRel, joinDivision, targetRelDataTypeIndexList.size(), joinDivision, rexBuilder);
    case INNER_JOIN_TO_RIGHT_JOIN :
    case LEFT_JOIN_TO_FULL_JOIN :
      return getNeedCompensateNullColumn(
          targetRel, 0, joinDivision, joinDivision, rexBuilder);
    case INNER_JOIN_TO_FULL_JOIN :
      return getNeedCompensateNullColumn(
          targetRel, 0, targetRelDataTypeIndexList.size(), joinDivision, rexBuilder);
    default : return null;
    }
  }
  public static List<Integer> getNeedCompensateNullColumn(
      MutableJoin targetRel, int leftIdx, int rightIdx, int joinDivision, RexBuilder rexBuilder) {
    return targetRel.rowType.getFieldList()
        .stream()
        .filter(typeField -> typeField.getIndex() >= leftIdx && typeField.getIndex() < rightIdx)
        .filter(typeField -> {
          RelDataTypeField relDataTypeField = typeField.getIndex() < joinDivision
              ? targetRel.getLeft().rowType.getFieldList().get(
                  typeField.getIndex())
              : targetRel.getRight().rowType.getFieldList().get(
                  typeField.getIndex() - joinDivision);
          return isOnlyNullableDifference(
              typeField.getValue(), relDataTypeField.getValue(), rexBuilder.getTypeFactory());
        }).map(RelDataTypeField::getIndex).collect(Collectors.toList());
  }
  public static boolean isOnlyNullableDifference(
      RelDataType queryRelDataType, RelDataType targetRelDataType,
      RelDataTypeFactory relDataTypeFactory) {
    requireNonNull(queryRelDataType, "queryRelDataType");
    requireNonNull(targetRelDataType, "targetRelDataType");
    if (queryRelDataType.equals(targetRelDataType)) {
      return false;
    }
    RelDataType newTargetRelDataType =
        relDataTypeFactory.createTypeWithNullability(
            targetRelDataType, queryRelDataType.isNullable());
   ...
   * dda.
   * @param queryType dfs.
   * @param targetType fdsf.
   * @return
   */
  public static @Nullable JoinRewriteType createJoinReWriteType(
      JoinRelType queryType, JoinRelType targetType) {
   ...
   * sdad.
   * @param rexNode dsfa.
   * @param rexBuilder sdfa.
   * @param typeList fds.
   * @param joinCompensateNullTypeColumn df.
   * @return
   */
  ...
  public static class JoinFixNullabilityShuttle
    ...
    @Override public RexNode visitInputRef(RexInputRef ref) {
      ...
      if (!joinCompensateNullTypeColumn.contains(ref.getIndex()) || refType.equals(rightType)) {
        return ref;
      }
  ...
  public static RexNode tryCastRexInputRef(
      RexNode rexNode, JoinContext joinContext, RexBuilder rexBuilder) {
    if (rexNode instanceof RexInputRef) {
      int index = ((RexInputRef) rexNode).getIndex();
      if (joinContext.getNeedCompensateNullColumn().contains(index)
          && isOnlyNullableDifference(rexNode.getType(),
          joinContext.getTargetDataTypeList().get(index), rexBuilder.getTypeFactory())) {
